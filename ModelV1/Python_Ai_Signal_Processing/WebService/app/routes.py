...
from flask import abort, flash, redirect # type: ignore
import matplotlib.pyplot as plt
from flask import Blueprint, logging, request, jsonify, render_template, redirect, send_from_directory, url_for, flash, session # type: ignore
...
import logging
import traceback
from pymongo.errors import ServerSelectionTimeoutError, OperationFailure, ConfigurationError # type: ignore
import time
import matplotlib
...
def connect_to_mongo():
    ...
    try:
        # Si estás en entorno Docker, sobrescribe DB_CLUSTER con "mongo"
        cluster = "mongo" if os.getenv("FLASK_ENV") == "docker" else db_cluster

        mongo_uri = f"mongodb://{db_username}:{db_password}@{cluster}:27017/"
...
def detect_noise():
    try:
        ...
        if 'Signal' not in data.columns:
            flash("'Signal' column not found in the uploaded file.", 'error')
            return redirect(url_for('main.index'))
